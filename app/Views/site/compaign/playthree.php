...
<?php 
		$wheels = 3;
		$spindata = getWheelDataOnSpin($wheels);
		$wheel_data = [
							$spindata['box_1'],
							$spindata['box_2'],
							$spindata['box_3'],
					...
					<div class="points-tb">
						<div class="d-flex justify-content-end align-items-center">
							<div class="ptb d-flex">
								...
								<div class="ptpp count" id="playedCoins"><?php print number_format( (new \App\Models\CompainModel())->getZapptaPlay($wheels, $pid) );?></div>
							</div>
							<div class="ptb d-flex">
								<div class="ptest">
								...
								<div class="ptpp count" id="playedCoinsWins"><?php print number_format( (new \App\Models\CompainModel())->getZapptaWinPlay($wheels, $pid) );?></div>
							</div>
							<div class="ptb d-flex">
								<div class="ptest">
								...
								<div class="ptpp count" id="playedCoinsTotal"><?php print number_format( (new \App\Models\CompainModel())->getTotalResult($wheels, $pid) );?></div>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="col-xxl-3 col-xl-3 col-lg-3 col-md-3 col-12">
					<div class="topPlayerBlock" id="topPlayerBlock">
						...
						<div class="tptable">
							...
							<?php 
								$points = (new \App\Models\CompainModel())->getUserCompaignResult($wheels,my_encrypt($single['com_id']),$single['store_id'],$single['pid']);
						...
						<div class="strike-block">
				    		<div class="position-relative d-flex">
			    				...
			    				<?php 
			    					$total_stikes_attend = (new \App\Models\CompainModel())->getTotlStrike($wheels, $single['compain_e_date'], $single['compain_s_date'], $pid);
...
<input type="hidden" id="winplayedCoins" value="<?=(new \App\Models\CompainModel())->getZapptaPlay($wheels, $pid)?>">
<input type="hidden" id="winplayedCoinsWins" value="<?=(new \App\Models\CompainModel())->getZapptaWinPlay($wheels, $pid)?>">
<input type="hidden" id="winplayedCoinsTotal" value="<?=(new \App\Models\CompainModel())->getTotalResult($wheels, $pid)?>">
...
<script type="text/javascript">
	...
	let colorsEven = ['#2D1F69', '#fadfae', '#2D1F69', '#fadfae'];
...
clickable.addEventListener("click", function() {
	...
	if ( wheelplayvalue == 0 ) {
	    ...
	    console.log('rdeg',rr);
	    console.log('wheelplayvalue',wheelplayvalue);
...
function addValuesResult()
{
	...
	if ( winpoerup == 1 ) {
		...
		var url = '<?php print base_url().'/compaign/play/'.$single['purl'].'/p/'.$single['pc'].'/'.'?sd_row='.$single['sd_row'].'&pds='.$single['pds'].'&playgive=1&wh=4';?>';
